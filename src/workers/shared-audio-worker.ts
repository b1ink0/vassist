...
class SharedAudioWorker {
    ...
    constructor() {
        ...
        this.isReady = false;
        this.isSharedWorker = typeof workerScope.onconnect !== 'undefined' || workerScope.constructor.name === 'SharedWorkerGlobalScope';
        ...
        if (this.isSharedWorker) {
            ...
            workerScope.onconnect = (event: MessageEvent) => {
                const eventWithPorts = event as MessageEvent & { ports?: MessagePort[] };
                ...
                port.onmessage = async (portEvent: MessageEvent<IncomingWorkerMessage>) => {
                    try {
            ...
            workerScope.onmessage = async (event: MessageEvent<IncomingWorkerMessage>) => {
                try {
                    // Wait for initialization to complete before processing
                    if (!this.isReady) {
                        Logger.log('SharedAudioWorker', 'Waiting for initialization...');
    ...
    private async handleMessage(message: IncomingWorkerMessage): Promise<WorkerSuccessResponse> {
     ...
     */
    private async handleProcessAudioWithLipSync(message: IncomingWorkerMessage): Promise<Record<string, unknown>> {
        const payload = message.data || {};
        const audioDataInput = payload.audioData;
        const sampleRate = typeof payload.sampleRate === 'number' ? payload.sampleRate : 24000;
        const originalAudioBuffer = payload.originalAudioBuffer;

        try {
            const pcmData = SharedAudioWorker.toFloat32Array(audioDataInput);
            const originalAudioBytes = SharedAudioWorker.toByteArray(originalAudioBuffer);

            Logger.log('SharedAudioWorker', `Processing audio: ${pcmData.length} samples @ ${sampleRate}Hz`);
     ...
     */
    private async handleVMDGenerate(message: IncomingWorkerMessage): Promise<Record<string, unknown>> {
        const payload = message.data || {};
        const audioDataInput = payload.audioData;
        const sampleRate = typeof payload.sampleRate === 'number' ? payload.sampleRate : 24000;
        const modelName = typeof payload.modelName === 'string' ? payload.modelName : 'Model';
     ...
     */
    private async handleKokoroInit(message: IncomingWorkerMessage): Promise<Record<string, unknown>> {
        const payload = message.data || {};
        const modelId = typeof payload.modelId === 'string' ? payload.modelId : 'onnx-community/Kokoro-82M-v1.0-ONNX';
        const device = (typeof payload.device === 'string' ? payload.device : 'wasm') as 'auto' | 'webgpu' | 'wasm';
        ...
        try {
            ...
            const initialized = await KokoroTTSCore.initialize({ modelId, device }, (progress) => {
            ...
            throw new Error(`Kokoro initialization failed: ${SharedAudioWorker.getErrorMessage(error)}`);
        }
    }
     ...
     */
    private async handleKokoroGenerate(message: IncomingWorkerMessage): Promise<Record<string, unknown>> {
        const payload = message.data || {};
        const text = typeof payload.text === 'string' ? payload.text : '';
        const voice = typeof payload.voice === 'string' ? payload.voice : 'af_heart';
        const speed = typeof payload.speed === 'number' ? payload.speed : 1.0;

        try {
            // Validate text parameter
            if (!text.trim()) {
                throw new Error(`Invalid text parameter: ${JSON.stringify(payload.text)}`);
            }
            ...
            throw new Error(`Kokoro generation failed: ${SharedAudioWorker.getErrorMessage(error)}`);
        }
    }
    ...
    private async handleKokoroCheckStatus(): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Kokoro status check failed: ${SharedAudioWorker.getErrorMessage(error)}`);
        }
    }
    ...
    private async handleKokoroListVoices(): Promise<{ voices: string[] }> {
        try {
            ...
            throw new Error(`Failed to list voices: ${SharedAudioWorker.getErrorMessage(error)}`);
        }
    }
    ...
    private async handleKokoroGetCacheSize(): Promise<Record<string, unknown>> {
        try {
            return await KokoroTTSCore.getCacheSize() as unknown as Record<string, unknown>;
        } catch (error: unknown) {
            Logger.error('SharedAudioWorker', 'Kokoro cache size check failed:', error);
            throw new Error(`Failed to get cache size: ${SharedAudioWorker.getErrorMessage(error)}`);
        }
    }
    ...
    private async handleKoroClearCache(): Promise<{ cleared: boolean }> {
        try {
            ...
            throw new Error(`Failed to clear cache: ${SharedAudioWorker.getErrorMessage(error)}`);
        }
    }
     ...
     */
    private async handleAudioProcess(message: IncomingWorkerMessage): Promise<Record<string, number>> {
        const payload = message.data || {};
        const sampleRate = typeof payload.sampleRate === 'number' ? payload.sampleRate : 24000;
    ...
    private static toFloat32Array(input: unknown): Float32Array {
        ...
        throw new Error('Invalid audio data payload; expected Float32Array or number[]');
    }
    ...
    private static toByteArray(input: unknown): Uint8Array {
        ...
        throw new Error('Invalid byte payload; expected Uint8Array, ArrayBuffer, or number[]');
    }
