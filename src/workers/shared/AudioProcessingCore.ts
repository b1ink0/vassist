...
export class AudioProcessingCore {
     ...
     */
    static async computeSpectrogram(audioData: Float32Array, sampleRate: number, frameRate = 30): Promise<SpectrogramResult> {
     ...
     */
    static analyzeVowelFrequencies(spectrogram: SpectrogramResult, vowelRanges: VowelRanges): VowelWeights[] {
        ...
        for (let frameIdx = 0; frameIdx < data.length; frameIdx++) {
            ...
            for (const [vowel, [lowFreq, highFreq]] of Object.entries(vowelRanges) as Array<[JapaneseVowel, [number, number]]>) {
     ...
     */
    static smoothVowelWeights(vowelWeights: VowelWeights[], windowSize = 15): VowelWeights[] {
        ...
        for (let i = 1; i < smoothed.length; i++) {
            ...
            for (const vowel of JAPANESE_VOWELS) {
                const newValue = alpha * currentSmoothed[vowel] + (1 - alpha) * previousSmoothed[vowel];
     ...
     */
    static adjustVowelWeights(weights: VowelWeights, config: VowelAdjustmentConfig): VowelWeights {
