...
const ControlPanel = ({
  ...
  const [queueStatus, setQueueStatus] = useState<AssistantQueueStatus>({
    length: 0,
    isEmpty: true,
    items: [],
  });
  ...
  useEffect(() => {
    const loadButtonPosition = async () => {
      ...
      try {
        const savedPos = (await StorageServiceProxy.configLoad(
          "devControlPanelButtonPosition",
          defaultPos,
        )) as { x?: number; y?: number };

        const buttonSize = 48;
        const rawX =
          typeof savedPos?.x === "number" ? savedPos.x : defaultPos.x;
        const rawY =
          typeof savedPos?.y === "number" ? savedPos.y : defaultPos.y;
        const boundedX = Math.max(
          10,
          Math.min(rawX, window.innerWidth - buttonSize - 10),
        );
        const boundedY = Math.max(
          10,
          Math.min(rawY, window.innerHeight - buttonSize - 10),
        );
        ...
        if (boundedX !== rawX || boundedY !== rawY) {
          await StorageServiceProxy.configSave(
            "devControlPanelButtonPosition",
            validPos,
          );
        }
    ...
    const handleResize = async () => {
      ...
      if (boundedX !== buttonPos.x || boundedY !== buttonPos.y) {
        ...
        try {
          ...
          Logger.error(
            "ControlPanel",
            "Failed to save button position on resize:",
            error,
          );
        }
      }
    ...
    const update = () => {
      try {
        ...
        const particles = scene.particleSystems
          ...
          : 0;
        const drawCalls =
          engine && engine._drawCalls && engine._drawCalls.current
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      if (hasDragged) {
        StorageServiceProxy.configSave(
          "devControlPanelButtonPosition",
          buttonPos,
        ).catch((error) => {
          Logger.error(
            "ControlPanel",
            "Failed to save button position after drag:",
            error,
          );
        });
  ...
  const testEmotion = async (emotionCategory: string, text: string) => {
    ...
    const placeholderMouthAnimationUrl = await ResourceLoader.getURLAsync(
      "res/private_test/motion/audio.bvmd",
    );
    ...
    try {
      ...
      Logger.error(
        "ControlPanel",
        "Failed to load placeholder animation:",
        error,
      );
    }
  };
  ...
  const playComposite = async (
    ...
    await assistantRef.current.playComposite(
      primaryAnimName,
      fillCategory,
      options,
    );
  ...
  const queueSimple = (animationName: string, force = false) => {
    ...
    Logger.log(
      "ControlPanel",
      `Queued animation: ${animationName} (force: ${force})`,
    );
    updateQueueStatus();
  };
  ...
  const queueSpeakTest = async (emotionCategory: string, force = false) => {
    ...
    try {
      const animationUrl = await ResourceLoader.getURLAsync(
        "res/private_test/motion/audio.bvmd",
      );
      ...
      Logger.log(
        "ControlPanel",
        `Queued speak: ${emotionCategory} (force: ${force})`,
      );
      updateQueueStatus();
    } catch (error) {
  ...
  const panelX = Math.max(
    10,
    Math.min(buttonPos.x - 450, window.innerWidth - 530),
  );
  ...
  return (
    <>
      <button
        ...
        className={cn(
          "w-12 h-12 bg-black/90 text-white border border-white/20 rounded-lg cursor-move shadow-lg backdrop-blur-sm flex items-center justify-center text-xl hover:bg-black transition-colors",
        ...
        >
          ...
          <div className="flex border-b border-white/20 bg-black/50 overflow-x-auto">
            <button
              ...
              className={cn(
                "flex-1 px-3 py-2 text-xs border-none cursor-pointer transition-colors",
                activeTab === "actions"
          ...
          <div className="p-2 md:p-4 max-h-[60vh] overflow-y-auto">
            ...
            {activeTab === "emotions" && (
              <div className="space-y-2">
                <p className="text-xs text-gray-400 mb-3">
                  Test emotion-based body language
                </p>
                <button
                  onClick={() =>
                    testEmotion("talking", "I am speaking normally!")
                  }
                  className="w-full px-3 py-2 bg-blue-500 text-white border-none rounded cursor-pointer text-xs text-left hover:bg-blue-600 transition-colors"
                >
                ...
                <button
                  onClick={() =>
                    testEmotion("thinking", "Let me think about that...")
                  }
                  className="w-full px-3 py-2 bg-purple-600 text-white border-none rounded cursor-pointer text-xs text-left hover:bg-purple-700 transition-colors"
                >
                ...
                <button
                  onClick={() =>
                    testEmotion("walking", "Let me explain while moving...")
                  }
                    ...
                    <div className="mt-2 space-y-1">
                      {queueStatus.items.map((item, index) => (
                        <p key={index} className="text-[10px] text-gray-400">
                          {index + 1}. {item.type} -{" "}
                          {item.animationName || item.primary || item.text}
                        </p>
                      ))}
                    </div>
                  )}
            ...
            {activeTab === "composite" && (
              <div className="space-y-3">
                <p className="text-xs text-gray-400 mb-3">
                  Primary animation + stitched fill animations
                </p>

                <div className="space-y-2">
                  <p className="text-xs text-white font-semibold">
                    Lip Sync + Body Animations
                  </p>
                  <button
            ...
            {activeTab === "debug" && (
              <div>
                <p className="text-xs text-gray-400 mb-3">
                  Camera and visualization controls
                </p>
                <DebugOverlay
            ...
            {activeTab === "logs" && (
              <div className="space-y-3">
                <p className="text-xs text-gray-400 mb-3">
                  Control console logging by category
                </p>
                ...
                <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
                  ...
                  <button
                    ...
                    className={cn(
                      "px-2 md:px-4 py-2 rounded-lg text-xs font-medium transition-colors",
                      loggerEnabled
                    ...
                    {logCategories
                      ...
                      .map(({ category, enabled, color }) => (
                        ...
                        >
                          ...
                          <button
                            onClick={async () => {
                              await Logger.setCategoryEnabled(
                                category,
                                !enabled,
                              );
                              setLogCategories(Logger.getCategories());
                            ...
                            className={cn(
                              "px-3 py-1 rounded text-[10px] font-medium transition-colors flex-shrink-0 ml-2",
                              enabled
