...
const DebugOverlay = ({ scene, positionManager }: DebugOverlayProps) => {
  ...
  const [coords, setCoords] = useState<{ x: string; y: string; scale: string }>(
    { x: "0", y: "0", scale: "12" },
  );
  ...
  const zoom = (direction: "in" | "out") => {
    ...
    const zoomAmount = 50;
    const newSize =
      direction === "in" ? currentSize + zoomAmount : currentSize - zoomAmount;
    const newWidth = newSize * 0.6;

    Logger.log(
      "DebugOverlay",
      `Zooming ${direction}: ${currentSize}px → ${newSize}px`,
    );
              ...
              <label className="flex items-center text-xs cursor-pointer hover:text-gray-300">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const pickingBox =
                      scene?.metadata?.animationManager?.pickingBox;
                    if (pickingBox) {
