...
const UISettings = ({
  ...
  const handleExportBackup = async (): Promise<void> => {
    ...
    try {
      ...
      const zipBlob = await appDataBackupService.exportToZip(
        getEffectiveBackupSelection(),
      );
  ...
  const handleImportBackup = async (
    ...
    try {
      ...
      await appDataBackupService.importFromZip(
        file,
        getEffectiveBackupSelection(),
      );
      ...
      setBackupStatus(
        "Backup import failed. Ensure this is a valid vassist backup ZIP.",
      );
      setBackupBusy(false);
    }
  };
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/70 mb-2">
              Need help? Check out the full documentation for setup guides,
      ...
      {isExtension && (
        ...
        >
          <Toggle
            ...
            onChange={(checked) =>
              updateUIConfig("autoLoadOnAllPages", checked)
            }
          />
        </SettingsRow>
      )}
          ...
          <div className="ml-4 mt-2">
            ...
            >
              <Toggle
                ...
                onChange={(checked) =>
                  updateUIConfig("enableColoredIconsToolbarOnly", checked)
                }
              />
            </SettingsRow>
          </div>
      ...
      <div className="space-y-4 border-t border-white/10 pt-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex-1">
              ...
              <p className="text-xs text-yellow-400/80 mt-0.5 flex items-start gap-1.5">
                ...
                />
                <span>
                  Performance Impact: Enables smooth height animation for
                  streaming text. May affect performance on lower-end devices.
                </span>
              </p>
            </div>
            <Toggle
        ...
        >
          <Toggle
            ...
            onChange={(checked) =>
              updateUIConfig("emotePlayback.showDurationBar", checked)
            }
          />
        </SettingsRow>
        ...
        {uiConfig.emotePlayback?.showDurationBar !== false && (
          <div className="ml-4 mt-2">
            <SettingsRow
              ...
              description={
                uiConfig.emotePlayback?.showTime !== false
            ...
            >
              <Toggle
                ...
                onChange={(checked) =>
                  updateUIConfig("emotePlayback.showTime", checked)
                }
              />
            </SettingsRow>
          </div>
        ...
        {(uiConfig.backgroundDetection?.mode ||
          BackgroundThemeModes.ADAPTIVE) === BackgroundThemeModes.ADAPTIVE && (
          <>
            <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-400/20 mb-3">
              <p className="text-xs text-blue-200/90 flex items-start gap-1.5">
                ...
                />
                <span>
                  <strong>Adaptive Mode:</strong> Automatically detects the page
                  background color and adjusts the assistant's theme for optimal
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-white/50">
                Sample grid size for background detection (3-10). Higher = more
                accurate. Default: 5
              </p>
            </div>
          </>
          ...
          <>
            ...
            >
              <Toggle
                ...
                onChange={(checked) =>
                  updateUIConfig("aiToolbar.showOnInputFocus", checked)
                }
              />
            </SettingsRow>
            ...
            >
              <Toggle
                ...
                onChange={(checked) =>
                  updateUIConfig("aiToolbar.showOnImageHover", checked)
                }
              />
            </SettingsRow>
          </>
      ...
      {!isAndroid ? (
        <div className="space-y-4 border-t border-white/10 pt-4">
          ...
          <ShortcutsConfig
            ...
            }) => updateUIConfig("shortcuts", shortcuts)}
        ...
        {useSelectiveBackup && (
          <div className="ml-4 grid grid-cols-1 md:grid-cols-2 gap-2">
            <SettingsRow label="Config" description="UI/AI/TTS/STT config keys">
              <Toggle
                checked={backupSelection.config}
                onChange={(checked) => updateBackupSelection("config", checked)}
              />
            </SettingsRow>
            <SettingsRow label="Settings" description="App settings namespace">
              <Toggle
                checked={backupSelection.settings}
                onChange={(checked) =>
                  updateBackupSelection("settings", checked)
                }
              />
            </SettingsRow>
            <SettingsRow
              label="Data / Presets"
              description="Generic app data records"
            >
              <Toggle
                checked={backupSelection.data}
                onChange={(checked) => updateBackupSelection("data", checked)}
              />
            </SettingsRow>
            <SettingsRow
              label="Chat History"
              description="Chats + media references"
            >
              <Toggle
                checked={backupSelection.chats}
                onChange={(checked) => updateBackupSelection("chats", checked)}
              />
            </SettingsRow>
            <SettingsRow label="Models" description="Avatar model files">
              <Toggle
                checked={backupSelection.models}
                onChange={(checked) => updateBackupSelection("models", checked)}
              />
            </SettingsRow>
            <SettingsRow label="Motions" description="Animation motion files">
              <Toggle
                checked={backupSelection.motions}
                onChange={(checked) =>
                  updateBackupSelection("motions", checked)
                }
              />
            </SettingsRow>
            <SettingsRow
              label="Emotes"
              description="Audio + motion emote bundles"
            >
              <Toggle
                checked={backupSelection.emotes}
                onChange={(checked) => updateBackupSelection("emotes", checked)}
              />
            </SettingsRow>
            <SettingsRow label="Stages" description="Stage model files">
              <Toggle
                checked={backupSelection.stages}
                onChange={(checked) => updateBackupSelection("stages", checked)}
              />
            </SettingsRow>
            <SettingsRow label="Voices" description="Saved reference voices">
              <Toggle
                checked={backupSelection.voices}
                onChange={(checked) => updateBackupSelection("voices", checked)}
              />
            </SettingsRow>
            <SettingsRow
              label="Backgrounds"
              description="Custom background assets"
            >
              <Toggle
                checked={backupSelection.backgrounds}
                onChange={(checked) =>
                  updateBackupSelection("backgrounds", checked)
                }
              />
            </SettingsRow>
            <SettingsRow
              label="Other Files"
              description="Any remaining file records"
            >
              <Toggle
                checked={backupSelection.otherFiles}
                onChange={(checked) =>
                  updateBackupSelection("otherFiles", checked)
                }
              />
            </SettingsRow>
          </div>
