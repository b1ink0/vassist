...
const GPTSoVITSConfig = ({
  ...
  const loadVoices = async () => {
    try {
      const allVoices =
        (await voiceStorageService.getAllVoices()) as VoiceItem[];
      setVoices(allVoices);
    } catch (error) {
  ...
  const handleSaveNewVoice = async () => {
    if (!newVoiceName || !newAudioFile || !newReferenceText) {
      setStatusMessage(
        "error-status:Please fill in all required fields (name, audio, reference text)",
      );
      return;
    }
    ...
    try {
      ...
      if (audioDuration > 10) {
        setStatusMessage(
          "error-status:Audio must be no longer than 10 seconds",
        );
        return;
      }
    } catch (error) {
      Logger.error(
        "GPTSoVITSConfig",
        "Failed to validate audio duration:",
        error,
      );
      setStatusMessage(
        "error-status:Failed to validate audio file. Please try a different file.",
      );
      return;
    }
    ...
    try {
      ...
      setStatusMessage(
        `error-status:Failed to upload voice: ${getErrorMessage(error)}`,
      );
    } finally {
  ...
  const handleEditVoice = async (voiceId: string) => {
    try {
      ...
      Logger.error(
        "GPTSoVITSConfig",
        "Failed to load voice for editing:",
        error,
      );
    }
  };
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className={cn(
              "glass-input w-full text-sm resize-none",
            ...
            <div
              className={cn(
                ...
                errorMessage.includes("✅")
                  ...
                  : errorMessage.startsWith("error-status:") ||
                      errorMessage.startsWith("hourglass:")
                    ...
                    {isEditing ? (
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          rows={2}
                          className={cn(
                            "glass-input w-full text-xs resize-none",
                            isLightBackground && "glass-input-dark",
                          )}
                        />
                        <Select
                      ...
                      <>
                        ...
                        <p className="text-xs text-white/50">
                          {(voice.audioData.size / 1024).toFixed(1)} KB •{" "}
                          {voice.language}
                        </p>
                      </>
                    )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onChange && onChange("speed", parseFloat(e.target.value))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onChange && onChange("topK", parseInt(e.target.value))
                    }
                    className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onChange && onChange("topP", parseFloat(e.target.value))
                    }
                    className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer"
                  />
                </div>
