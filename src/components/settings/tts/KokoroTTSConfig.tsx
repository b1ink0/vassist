 ...
 */
const getVoiceGroups = (): Array<{
  label: string;
  voices: VoiceGroupOption[];
}> => {
...
const getDeviceOptions = (): Array<{
  ...
  const descriptions: Record<string, { label: string; description: string }> = {
    [KokoroDevice.AUTO]: {
      ...
      description: "Automatically chooses the best available backend",
    },
    [KokoroDevice.WEBGPU]: {
      label: "WebGPU",
      description: "GPU acceleration (2-10x faster, fp32 precision)",
    },
    [KokoroDevice.WASM]: {
      label: "WASM",
      description: "CPU-based (slower but more stable, q8 quantized)",
    },
  };
    ...
    <div className="space-y-4 p-2 md:p-4 rounded-lg bg-white/5 border border-white/10">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-blue-100">
              ...
              <ol className="list-decimal list-inside space-y-2 text-blue-200/90 ml-2">
                <li>
                  <strong>Choose a Voice:</strong> Select from the dropdown
                  below. Try different voices to find your favorite!
                </li>
                <li>
                  <strong>Adjust Speed:</strong> Use the slider to make speech
                  faster or slower (1.0x is normal speed).
                </li>
                <li>
                  <strong>Pick Device Backend:</strong> Auto is recommended.
                  WebGPU uses ~350MB (faster), WASM uses ~86MB (slower but more
                  compatible).
                </li>
                <li>
                  <strong>Download the Model:</strong> Click "Initialize Model"
                  to download. Size depends on backend selected.
                </li>
                <li>
                  <strong>Test Your Voice:</strong> After initialization, click
                  "Test Voice" to hear a sample.
                </li>
              </ol>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2 text-sm text-yellow-100">
                <p className="font-semibold">
                  Important Performance & Compatibility Notes
                </p>
                <ol className="list-decimal list-inside space-y-2 text-yellow-200/90 ml-2">
                  <li className="pl-2">
                    <strong>Page Lag:</strong> If you experience lag or poor
                    performance with WebGPU or Auto backend, switch to{" "}
                    <strong>WASM backend</strong> (in Advanced Settings below)
                    or choose a different TTS provider.
                  </li>
                  <li className="pl-2">
                    <strong>Gibberish Audio:</strong> If the generated audio
                    sounds like gibberish or is unintelligible while using
                    WebGPU or Auto, your system doesn't support WebGPU properly.
                    Switch to <strong>WASM backend</strong> to fix this issue.
                  </li>
                  <li className="pl-2">
                    <strong>Backend Comparison:</strong> WebGPU is 2-10x faster
                    but requires GPU support and uses more memory (~350MB). WASM
                    is slower but more stable, compatible with all systems, and
                    uses less memory (~86MB).
                  </li>
                </ol>
              </div>
            </div>
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-white/90">
          Voice{" "}
          {isSetupMode && (
            <span className="text-xs text-white/60">(Step 1)</span>
          )}
        </label>
        {isSetupMode && (
          <p className="text-xs text-white/60 mb-2">
            Choose the voice that will speak to you. Each voice has a unique
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-white/90">
          Speech Speed: {(config.speed || 1.0).toFixed(1)}x{" "}
          {isSetupMode && (
            <span className="text-xs text-white/60">(Step 2)</span>
          )}
        </label>
        {isSetupMode && (
          <p className="text-xs text-white/60 mb-2">
            Drag the slider to adjust how fast or slow the voice speaks. 1.0x is
            normal speed.
          </p>
        )}
        <input
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-white/90">
          Device Backend{" "}
          {isSetupMode && (
            <span className="text-xs text-white/60">(Step 3 - Optional)</span>
          )}
        </label>
        {isSetupMode && (
          <p className="text-xs text-white/60 mb-2">
            Choose how the model runs: <strong>Auto</strong> (recommended)
            automatically picks the best option.
            <strong>WebGPU</strong> is faster (~350MB download) but needs a good
            graphics card.
            <strong>WASM</strong> is slower but smaller (~86MB) and works on any
            device.
          </p>
        )}
        <Select
        ...
        {isSetupMode && (
          <div className="mt-2 p-2 rounded-lg bg-blue-500/10 border border-blue-400/20">
            <p className="text-xs text-blue-200/90 flex items-start gap-1.5">
              ...
              />
              <span>
                <strong>Tip:</strong> Select "Auto" to let the system choose the
                best option for your device. If you have a gaming PC or laptop
                with a dedicated graphics card, it will use WebGPU (faster).
                Otherwise, it will use WASM (smaller download, works
                everywhere).
              </span>
            </p>
          </div>
        ...
        {!isSetupMode && (
          <>
            <p className="text-xs text-white/50">
              Quantization is automatic: WebGPU uses fp32 (~350MB), WASM uses q8
              (~86MB)
            </p>
            <div className="mt-2 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30 flex items-start gap-2">
              ...
              <p className="text-xs text-yellow-200/90 leading-relaxed">
                <strong>Performance Note:</strong> If the model or page lags,
                try switching to WASM backend for better stability. However,
                WASM is significantly slower than WebGPU. For optimal
                performance, consider using different TTS providers (OpenAI,
                OpenAI-Compatible) or hosting a TTS service directly on your
                device.
              </p>
            </div>
          </>
        ...
        <div className="space-y-4 pt-2">
          ...
          {!isSetupMode && (
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-white/50">
                HuggingFace model ID (Leave empty to use default:
          ...
          <div className="space-y-2">
            <label className="flex items-center justify-between cursor-pointer">
              <div>
                ...
                <p className="text-xs text-white/50">
                  Periodically generates audio to keep model in memory (prevents
                  unload lag)
                </p>
              </div>
              <Toggle
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-purple-100">
                  ...
                  <p className="text-purple-200/90">
                    Click the "Initialize Model" button below to download Kokoro
                    TTS. Download size: <strong>~86MB for WASM</strong> or{" "}
                    <strong>~350MB for WebGPU</strong> (Auto chooses best for
                    your device). This only happens once - the model is saved on
                    your device for offline use. The download may take a few
          ...
          <div className="flex items-center justify-between gap-3">
            <Button
              ...
              className={cn(
                ...
                isSetupMode && !kokoroStatus.initialized
                  ? "ring-2 ring-purple-400 ring-offset-2 ring-offset-transparent animate-pulse"
                  : "",
              )}
            >
              {kokoroStatus.checking ? (
      ...
      {showTestButton && onTestVoice && kokoroStatus?.initialized && (
        <>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-green-100">
                  ...
                  <p className="text-green-200/90">
                    Click "Test Voice" below to hear a sample with your selected
                    voice and speed. This helps you confirm everything is
                    working before continuing. You can change the voice or speed
                    and test again until you find your perfect settings!
                  </p>
                </div>
              </div>
            </div>
          ...
          <p className="text-xs text-blue-200/90 flex items-start gap-1.5">
            ...
            />
            <span>
              If you hear gibberish, switch to <strong>WASM backend</strong> or
              turn <strong>OFF</strong> hardware acceleration in Chrome
              settings.
            </span>
          </p>
        </>
