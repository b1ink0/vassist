...
const VitsModelDownloader = ({ androidAPI, isLightBackground = false }) => {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      if (result?.success) {
        setStatus(result.status?.vits);
      }
    } catch (err) {
      console.error('Failed to load VITS status:', err);
    }
  }, [androidAPI]);
  ...
  useEffect(() => {
    ...
    androidAPI._onSTTTTSComplete = (modelType, result) => {
      if (modelType === 'vits') {
        ...
        setSuccess('VITS TTS model downloaded successfully!');
  ...
  const handleDownload = async () => {
    ...
    try {
      const result = JSON.parse(androidAPI.downloadVitsModel());
      if (!result?.success) {
  ...
  const handleDelete = async () => {
    if (!androidAPI) return;
    if (!confirm('Delete VITS TTS model?\n\nThis will free up ~152 MB of storage.')) return;
    
    try {
      const result = JSON.parse(androidAPI.deleteVitsModel());
      if (result?.success) {
  ...
  return (
    <div className="p-4 rounded-lg bg-white/5 border border-white/10 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h4 className="text-sm font-semibold text-white/90">VITS Model</h4>
      </div>
