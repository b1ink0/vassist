...
const VitsModelDownloader = ({ androidAPI, isLightBackground = false }: VitsModelDownloaderProps) => (
  <ModelDownloader
    androidAPI={androidAPI}
    isLightBackground={isLightBackground}
    modelType="vits"
    statusKey="vits"
    downloadFn={() => androidAPI?.downloadVitsModel?.() ?? '{"success":false,"error":"VITS download API unavailable"}'}
    deleteFn={() => androidAPI?.deleteVitsModel?.() ?? '{"success":false,"error":"VITS delete API unavailable"}'}
    title="VITS Model"
    downloadSize="~145 MB"
