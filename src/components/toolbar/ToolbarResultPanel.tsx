  ...
  (
    ...
    const getActionInfo = () => {
      if (action?.startsWith("summarize-"))
        return { icon: "note", label: "Summary" };
      if (action === "translate")
        return { icon: "globe", label: "Translation" };
      if (action === "detect-language")
        return { icon: "search", label: "Language Detection" };
      if (action === "dictation")
        return { icon: "microphone", label: "Dictation" };
      if (action === "write") return { icon: "edit", label: "Writer" };
      if (action?.startsWith("rewrite-"))
        return { icon: "refresh", label: "Rewrite" };
      if (action === "image-describe")
        return { icon: "image", label: "Description" };
      if (action === "image-extract-text")
        return { icon: "document", label: "Extracted Text" };
      if (action === "image-identify-objects")
        return { icon: "tag", label: "Objects" };
      if (action === "dictionary-define")
        return { icon: "book", label: "Definition" };
      if (action === "dictionary-synonyms")
        return { icon: "refresh", label: "Synonyms" };
      if (action === "dictionary-antonyms")
        return { icon: "bidirectional", label: "Antonyms" };
      if (action === "dictionary-pronunciation")
        return { icon: "speaker", label: "Pronunciation" };
      if (action === "dictionary-examples")
        return { icon: "idea", label: "Examples" };
      if (action === "improve-grammar")
        return { icon: "check", label: "Grammar" };
      if (action === "improve-spelling")
        return { icon: "check", label: "Spelling" };
      if (action === "improve-professional")
        return { icon: "briefcase", label: "Professional" };
      if (action === "improve-formal")
        return { icon: "formal", label: "Formal" };
      if (action === "improve-simplify")
        return { icon: "book", label: "Simplified" };
      if (action === "improve-expand")
        return { icon: "note", label: "Expanded" };
      if (action === "improve-concise")
        return { icon: "lightning", label: "Concise" };
    ...
    return (
      ...
      >
        ...
        {/* Error display */}
        {error && !isLoading && (
          <div className="text-[13px] leading-6 text-[#ff6b6b]">{error}</div>
        )}
