  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      const shouldUpdate =
        ...
        targetWordCount >= wordsArrayRef.current.length;
  ...
  useEffect(() => {
    ...
    const updateHeight = () => {
      if (contentRef.current) {
        ...
        if (diff > 0.5) {
          const step = Math.max(
            (targetHeightRef.current - currentHeightRef.current) *
              TRANSITION_SPEED,
            MIN_STEP,
          );
          ...
          if (
            Math.abs(currentHeightRef.current - targetHeightRef.current) < 0.5
          ) {
            currentHeightRef.current = targetHeightRef.current;
          }
          ...
          diff > 0 &&
          Math.abs(currentHeightRef.current - lastSetHeight) > 0.1
        ) {
          currentHeightRef.current = targetHeightRef.current;
          lastSetHeight = currentHeightRef.current;
          setContainerHeight(currentHeightRef.current + "px");
    ...
    >
      ...
      >
        {displayedWords.map((word, index) => {
          ...
          return (
            <span key={index} className={animationClass}>
              {word}
            </span>
          );
        })}
