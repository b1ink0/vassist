...
const getModelSizePx = (
  config: UIConfigForScene | null | undefined,
): PixelSize | null => {
...
const isWallpaperVisibilityDetail = (
  value: unknown,
): value is WallpaperVisibilityDetail => {
  if (!value || typeof value !== "object") {
    return false;
  }

  return (
    typeof (value as Partial<WallpaperVisibilityDetail>).visible === "boolean"
  );
};
...
const BabylonScene = ({
    ...
    if (isDesktop && typeof window !== "undefined") {
      if (modelSizePx) {
        ...
        const width = Math.max(
          baseWidth + modelWidthDelta * scaleFactorWidth,
          400,
        );
        const height = Math.max(
          baseHeight + modelHeightDelta * scaleFactorHeight,
          525,
        );
        Logger.log(
          "BabylonScene",
          `Initializing canvas with saved size: ${width}x${height}`,
        );
        return { width, height };
      }
      return {
  ...
  useEffect(() => {
    const handleCanvasSizeUpdate = (event: Event): void => {
      if (
        ...
        !isCanvasDimensions(event.detail)
      ) {
        return;
      }
  ...
  useEffect(() => {
    ...
    if (modelSizePx) {
      ...
      Logger.log(
        "BabylonScene",
        `Applying saved zoom to Electron window on mount: ${modelWidth}x${modelHeight}`,
      );
      ...
      updateWindowSizeForZoom(modelWidth, modelHeight)
        .then(() => {
        ...
        .then((windowSize: { width: number; height: number }) => {
          Logger.log(
            "BabylonScene",
            `Electron window resized to: ${windowSize.width}x${windowSize.height}`,
          );
          ...
          setTimeout(() => {
            if (activePositionManagerRef?.current) {
              ...
              Logger.log(
                "BabylonScene",
                `Position maintained at ratio (${posXRatio.toFixed(2)}, ${posYRatio.toFixed(2)}): (${newPosX}, ${newPosY})`,
              );
            } else {
              Logger.warn(
                "BabylonScene",
                "PositionManager not ready yet, will update on next zoom",
              );
            }
          }, 200);
        })
        .catch((err: unknown) => {
          Logger.warn(
            "BabylonScene",
            "Failed to resize Electron window on mount:",
            err,
          );
        });
    }
  ...
  const onSceneReadyRef =
    useRef<BabylonSceneProps["onSceneReady"]>(onSceneReady);
  const onLoadProgressRef =
    useRef<BabylonSceneProps["onLoadProgress"]>(onLoadProgress);
  const sceneConfigRef = useRef<Partial<SceneBuildConfig>>(sceneConfig);
  const forceChatOnlyModeRef = useRef(forceChatOnlyMode);

  useEffect(() => {
    onSceneReadyRef.current = onSceneReady;
    onLoadProgressRef.current = onLoadProgress;
    sceneConfigRef.current = sceneConfig;
    forceChatOnlyModeRef.current = forceChatOnlyMode;
  }, [onSceneReady, onLoadProgress, sceneConfig, forceChatOnlyMode]);
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        Logger.log(
          "BabylonScene",
          "Tab visible and scene ready - hiding loading overlay",
        );
  ...
  useEffect(() => {
    ...
    Logger.log(
      "BabylonScene",
      "Canvas ref ready, checking if in DOM...",
      canvas.parentNode ? "YES" : "NO",
    );
    ...
    if (!canvas.parentNode) {
      Logger.log(
        "BabylonScene",
        "Canvas not in DOM yet, skipping initialization",
      );
      return;
    }
    ...
    const initEngine = async (): Promise<(() => void) | null> => {
      ...
      if (!canvas.parentNode) {
        Logger.log(
          "BabylonScene",
          "Canvas removed from DOM during initialization, aborting",
        );
        delete canvas.dataset.babylonInitializing;
        return null;
      }
      ...
      Logger.log(
        "BabylonScene",
        `Device pixel ratio: ${rawDPR}, effective: ${effectiveDPR}, isAndroid: ${isAndroid}`,
      );
      ...
      if (sceneBuilder) {
        ...
        const baseSceneConfig =
          (await getSceneConfigAsync()) as SceneBuildConfig;
        Logger.log(
          "BabylonScene",
          "getSceneConfigAsync() returned, finalConfig.modelUrl:",
          baseSceneConfig.modelUrl,
        );
        ...
        Logger.log(
          "BabylonScene",
          "After merge, finalConfig.modelUrl:",
        ...
        try {
          ...
          Logger.error(
            "BabylonScene",
            "Scene initialization failed, switching to chat-only mode:",
            error,
          );

          setShowModelLoadingOverlay(false);
          forceChatOnlyModeRef.current("scene-build-failed");
        ...
        if (cancelled) {
          Logger.log(
            "BabylonScene",
            "Initialization cancelled after scene building",
          );
      ...
      const handleWallpaperVisibility = (event: Event): void => {
        if (
          !(event instanceof CustomEvent) ||
          !isWallpaperVisibilityDetail(event.detail)
        ) {
          return;
        }
    ...
    return () => {
      ...
      if (canvasToRemove && canvasToRemove.parentNode) {
        setTimeout(() => {
          if (
            canvasToRemove.dataset.babylonInitialized !== "true" &&
  ...
  useEffect(() => {
    ...
    const updatePosition = (): void => {
      try {
        ...
        const modelPos: PositionPixels =
          activePositionManager.getPositionPixels();
        setModelOverlayPos({
          x: modelPos.x,
          y: modelPos.y,
  ...
  useEffect(() => {
    ...
    dragDropService.attach(overlayElement, {
      ...
      onShowError: (error: unknown) =>
        Logger.error("BabylonScene", "Drag-drop error:", error),
      checkVoiceMode: null,
      getCurrentCounts: () => ({ images: 0, audios: 0 }),
      onProcessData: (data: unknown) => {
        Logger.log(
          "BabylonScene",
          "Opening chat from model drop with data:",
          data,
        );
        openChat();
  ...
  const canvasHeight =
    isAndroid && initialHeightRef.current !== null
