...
const EmotePlaybackBar = ({
  ...
  const queueSeekUpdate = (nextProgress: number): void => {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const normalized = clamp((event.clientX - trackRect.left) / trackRect.width, 0, 1);
  ...
  const surfaceClass = cn('glass-button', useDarkGlassVariant && 'glass-button-dark');
  ...
  const panelStyle = useMemo(() => {
    const top = isAndroidPlatform
      ? clamp(viewport.height - (isChatOpen ? 100 : 110), 10, viewport.height - 76)
      : clamp(
          (modelAnchor ? modelAnchor.y + modelAnchor.height + 14 : viewport.height - 94),
          10,
          viewport.height - 76
        );

    const left = isAndroidPlatform
      ? clamp((viewport.width - panelWidth) / 2, 10, viewport.width - panelWidth - 10)
      : clamp(
          (modelAnchor ? modelAnchor.x + modelAnchor.width / 2 : viewport.width / 2) - panelWidth / 2,
          10,
  ...
  }, [isAndroidPlatform, isChatOpen, modelAnchor, panelWidth, viewport.height, viewport.width]);
  ...
  return (
    ...
    >
      ...
      <div className={cn('mt-2 grid grid-cols-3 items-center gap-2 font-medium leading-none', timeRowHeight, isAndroidUi ? 'text-xs' : 'text-[11px]', textClass)}>
        <div className="flex justify-start">
          {showTime ? (
            <span className={cn('inline-flex items-center rounded-full px-2 leading-none', surfaceClass, timePillHeight)}>
        ...
        <div className="flex justify-center items-center">
          <Button
            ...
            aria-label={isPaused ? 'Resume emote playback' : 'Pause emote playback'}
          >
            <Icon
