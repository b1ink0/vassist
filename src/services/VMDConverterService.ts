...
class VMDConverterService {
  ...
  async convertVMDToBVMD(
    ...
    try {
      Logger.log(
        "VMDConverter",
        `Converting VMD: ${vmdFile.name} (${vmdFile.size} bytes)`,
      );
      ...
      if (!conversionScene) {
        Logger.log(
          "VMDConverter",
          "No scene provided, creating conversion scene...",
        );
      ...
      try {
        ...
        const { BvmdConverter } =
          await import("babylon-mmd/esm/Loader/Optimized/bvmdConverter");
        ...
        Logger.log(
          "VMDConverter",
          `BVMD created (${bvmdArrayBuffer.byteLength} bytes)`,
        );
      ...
      Logger.error(
        "VMDConverter",
        `Failed to convert ${vmdFile.name}:`,
        normalized,
      );
  ...
  async convertBatch(
    ...
    Array<{
      filename: string;
      bvmdData: ArrayBuffer | null;
      error: string | null;
    }>
  > {
    Logger.log(
      "VMDConverter",
      `Starting batch conversion of ${vmdFiles.length} VMD files`,
    );
    ...
    if (!sharedScene) {
      Logger.log(
        "VMDConverter",
        "Creating shared conversion scene for batch...",
      );
  ...
  async validateVMD(vmdFile: File): Promise<boolean> {
    try {
      ...
      if (!isValid) {
        Logger.warn(
          "VMDConverter",
          `Invalid VMD header detected: ${vmdFile.name}`,
        );
      }
   ...
   */
  async getVMDInfo(vmdFile: File): Promise<{
    isValid: boolean;
    version: string;
    modelName: string;
    fileSize: number;
