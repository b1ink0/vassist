...
class FrameCaptureService {
  ...
  registerProvider(provider: CaptureProvider | null): boolean {
    if (!provider || typeof provider.captureFrame !== "function") {
      Logger.error(
        "FrameCaptureService",
        "Invalid provider - must have captureFrame() method",
      );
      return false;
    }
  ...
  async getLatestFrame(): Promise<CaptureResult> {
    ...
    if (this.isCapturing) {
      Logger.warn(
        "FrameCaptureService",
        "Capture already in progress, skipping",
      );
      return {
        success: false,
        frame: null,
    ...
    try {
      ...
      const frame = await Promise.race<string | null>([
        capturePromise,
        timeoutPromise,
      ]);
      ...
      if (!frame) {
        Logger.warn(
          "FrameCaptureService",
          "Provider returned null/empty frame",
        );
        return {
          success: false,
          frame: null,
      ...
      if (typeof frame !== "string" || !frame.startsWith("data:image/")) {
        Logger.error(
          "FrameCaptureService",
          "Invalid frame format - expected data URL",
        );
        return {
          success: false,
          frame: null,
  ...
  getProviderInfo():
    | { registered: false }
    | { registered: true; name: string; type: string } {
    if (!this.provider) {
