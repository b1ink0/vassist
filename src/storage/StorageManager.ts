...
interface ConfigNamespace {
  ...
  load: <T = unknown>(key: string, defaultValue?: T | null) => Promise<T | null | unknown>;
...
interface FilesNamespace {
  ...
  getMetadataByCategory: (category: string) => Promise<Record<string, UnknownRecord>>;
  clear: () => Promise<boolean>;
}
...
export class StorageManager {
  ...
  constructor() {
    ...
    this.config = {
      ...
      load: <T = unknown>(key: string, defaultValue: T | null = null) => this._configLoad<T>(key, defaultValue),
    ...
    this.settings = {
      ...
      load: <T = unknown>(key: string, defaultValue: T | null = null) => this._settingsLoad<T>(key, defaultValue),
    ...
    this.cache = {
      save: (key: string, value: unknown, ttlSeconds = 3600) => this._cacheSave(key, value, ttlSeconds),
    ...
    this.chat = {
      save: (chatId: string, data: UnknownRecord) => this._chatSave(chatId, data),
    ...
    this.files = {
      save: (fileId: string, data: unknown, category = 'general') => this._fileSave(fileId, data, category),
      ...
      getMetadataByCategory: (category: string) => this._filesGetMetadataByCategory(category),
      clear: () => this._filesClear(),
    };

    // Namespace for generic data storage by category
    this.data = {
      save: (key: string, value: unknown, category = 'general') => this._dataSave(key, value, category),
  ...
  private async _configLoad<T = unknown>(key: string, defaultValue: T | null = null): Promise<T | null | unknown> {
  ...
  private async _settingsLoad<T = unknown>(key: string, defaultValue: T | null = null): Promise<T | null | unknown> {
  ...
  private async _cacheSave(key: string, value: unknown, ttlSeconds = 3600): Promise<boolean> {
    const now = new Date();
  ...
  private async _chatSave(chatId: string, data: UnknownRecord): Promise<boolean> {
  ...
  private async _fileSave(fileId: string, data: unknown, category = 'general'): Promise<boolean> {
  ...
  private async _filesGetByCategory(category: string): Promise<Record<string, unknown>> {
  ...
  private async _filesGetMetadataByCategory(category: string): Promise<Record<string, UnknownRecord>> {
    const records = await this.adapter.query('files', { category });
  ...
  private async _dataSave(key: string, value: unknown, category = 'general'): Promise<boolean> {
  ...
  private async _dataGetByCategory(category: string): Promise<Record<string, unknown>> {
