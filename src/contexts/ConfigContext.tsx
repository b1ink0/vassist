...
const setConfigValueAtPath = <T extends object>(
  config: T,
  path: string,
  value: unknown,
): T => {
...
export const ConfigProvider = ({ children }: ConfigProviderProps) => {
  ...
  const scheduleConfigPathUpdate = useCallback(
    <T extends object>(
      setState: React.Dispatch<React.SetStateAction<T>>,
      debouncersRef: React.MutableRefObject<
  ...
  useEffect(() => {
    return () => {
      [
        aiUpdateDebouncersRef,
        ttsUpdateDebouncersRef,
        sttUpdateDebouncersRef,
        uiUpdateDebouncersRef,
      ].forEach((debouncersRef) => {
        Object.values(debouncersRef.current).forEach((item) =>
          item.debounced.cancel(),
        );
  ...
  const syncDesktopServerForProviders = useCallback(
    async (
      nextAiConfig: AIConfig,
      nextTtsConfig: TTSConfig,
      nextSttConfig: STTConfig,
    ) => {
      if (!isDesktop || !api?.server) {
        return;
      }

      const llmUsesDesktopLocal =
        nextAiConfig?.provider === AIProviders.DESKTOP_LOCAL;
      const ttsUsesDesktopLocal =
        nextTtsConfig?.provider === TTSProviders.DESKTOP_LOCAL;
      const sttUsesDesktopLocal =
        nextSttConfig?.provider === STTProviders.DESKTOP_LOCAL;
      ...
      if (!needsDesktopProxy) {
        try {
          ...
          Logger.warn(
            "ConfigContext",
            "Desktop proxy stop skipped/failed:",
            error,
          );
        }
        return;
      ...
      try {
        ...
        Logger.error(
          "ConfigContext",
          "Error starting desktop proxy server:",
          error,
        );
      }
    },
    ...
    const loadConfigs = async () => {
      ...
      try {
        ...
        try {
          ...
          Logger.warn(
            "ConfigContext",
            "Failed to configure AI Service:",
            error,
          );
        }
  ...
  const updateUIConfig = useCallback(
    (path: string, value: unknown, options?: ConfigUpdateOptions) => {
  ...
  const saveUIConfig = useCallback(async () => {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("uiConfigUpdated", { detail: uiConfig }),
      );
  ...
  const saveAIConfig = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setAiConfigError(
          "Failed to configure AI service: " + getErrorMessage(error),
        );
  ...
  const saveTTSConfig = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setTtsConfigError(
          "Failed to configure TTS service: " + getErrorMessage(error),
        );
  ...
  const checkKokoroStatus = useCallback(
    async (desiredDeviceOverride: string | null = null) => {
      ...
      try {
        ...
        const desiredDevice =
          desiredDeviceOverride || ttsConfig.kokoro?.device || "auto";
  ...
  const initializeKokoro = useCallback(async () => {
    try {
      ...
      Logger.log(
        "ConfigContext",
        "Initializing Kokoro with device:",
        ttsConfig.kokoro?.device,
        "Full kokoro config:",
        ttsConfig.kokoro,
      );
  ...
  const testTTSConnection = useCallback(
    async (customText: string | null = null) => {
      setTtsConfigError("");
      setTtsTesting(true);
      ...
      try {
        ...
        if (ttsConfig.provider === TTSProviders.KOKORO) {
          ...
          if (!status.initialized) {
            ...
            Logger.log(
              "ConfigContext",
              "Auto-initializing Kokoro for test TTS",
            );
        ...
        if (ttsConfig.provider === TTSProviders.KOKORO) {
          setTtsConfigError(
            `✅ TTS test successful! Generated in ${duration}s using voice: ${ttsConfig.kokoro?.voice || "default"}`,
          );
        } else if (ttsConfig.provider === TTSProviders.ANDROID_LOCAL) {
          setTtsConfigError(
            `✅ Android TTS test successful! Generated in ${duration}s using voice: ${ttsConfig["android-local"]?.voice || "default"}`,
          );
        } else {
  ...
  const saveSTTConfig = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setSttConfigError(
          "Failed to configure STT service: " + getErrorMessage(error),
        );
  ...
  const testSTTRecording = useCallback(
  ...
  const startChromeAIDownload = useCallback(async () => {
    try {
      ...
      const status =
        (await checkChromeAIAvailability()) as ChromeAvailabilityStatus;
      ...
      if (
        ...
        Logger.log(
          "ConfigContext",
          "Model not in downloadable state:",
          status.state,
        );
      ...
      const result = await downloadChromeAIModelWithProgress((progress) => {
        Logger.log(
          "ConfigContext",
          `Chrome AI download progress: ${progress.progress?.toFixed(1)}%`,
        );
        setChromeAiStatus((prev) => ({
          ...prev,
      ...
      if (result && result.success) {
        setChromeAiStatus((prev) => ({
          ...
          details:
            "Please check chrome://on-device-internals for progress, then refresh status",
        }));
        ...
        setChromeAiStatus((prev) => ({
          ...
          details:
            result.details ||
            "Please check chrome://on-device-internals for status",
        }));
      } else {
      ...
      Logger.log(
        "ConfigContext",
        "Download monitor completed, rechecking availability...",
      );
  ...
  const testLanguageDetector = useCallback(
    async (text: string) => {
      if (!aiConfig.aiFeatures?.languageDetector?.enabled) {
  ...
  const testSummarizer = useCallback(
  ...
  const testRewriter = useCallback(
    async (text: string) => {
      if (!aiConfig.aiFeatures?.rewriter?.enabled) {
