  ...
  blink: [
    {
      ...
      metadata: {
        description:
          "Eye blink animation - contains only eye morph tracks for natural blinking",
...
export function validateAnimationConfig(
  config:
    | { id?: unknown; filePath?: unknown; category?: unknown }
    | null
    | undefined,
) {
...
export const StateTransitions = {
  ...
  [AssistantState.CELEBRATING]: [AssistantState.IDLE, AssistantState.COMPOSITE],
...
export function getAnimationForEmotion(emotion: string | null | undefined) {
  if (!emotion) {
    Logger.warn(
      "AnimationConfig",
      "No emotion provided, using default (neutral)",
    );
  ...
  if (!hasOwnKey(EmotionMapping, normalizedEmotion)) {
    Logger.warn(
      "AnimationConfig",
      `Unknown emotion: "${emotion}", using default (neutral)`,
    );
    Logger.warn(
      "AnimationConfig",
      `Available emotions: ${Object.keys(EmotionMapping).join(", ")}`,
    );
    return getAnimationById(EmotionMapping.default);
  }
  ...
  if (!animation) {
    Logger.error(
      "AnimationConfig",
      `Animation not found for emotion: "${emotion}" (ID: ${animationId})`,
    );
    Logger.error(
      "AnimationConfig",
      "This indicates a configuration error - emotion maps to non-existent animation",
    );
    return getAnimationById(EmotionMapping.default);
  }
...
export function sanitizeEmotion(emotion: unknown): string {
  ...
  if (typeof emotion !== "string") {
    Logger.warn(
      "AnimationConfig",
      `Emotion is not a string (type: ${typeof emotion}), using default`,
    );
    return "default";
  }
  ...
  if (!(normalized in EmotionMapping)) {
    Logger.warn(
      "AnimationConfig",
      `Invalid emotion "${emotion}" from LLM, using default`,
    );
    return "default";
  }
...
export function isValidTransition(
  fromState: AssistantStateValue,
  toState: AssistantStateValue,
) {
