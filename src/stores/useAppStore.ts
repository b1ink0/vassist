...
const scheduleChatAutoSave = () => {
  ...
  if (state.chatMessages.length === 0 || state.isTempChat || state.isProcessing) {
    return;
  }
  ...
  chatAutoSaveTimeout = setTimeout(async () => {
    ...
    try {
      ...
      Logger.error("AppStore", "Failed to auto-save chat (debounced):", error);
    }
  }, 2000);
};
...
export const useAppStore = create<AppStoreState>()(
  devtools(
    (set, get) => ({
      ...
      handleAssistantReady: ({ animationManager: _animationManager, positionManager, scene }) => {
      ...
      setIsAssistantReady: (value) => {
        set((state) => ({
          isAssistantReady: resolveSetStateAction(value, state.isAssistantReady),
        }));
      },
      ...
      setIsChatInputVisible: (value) => {
        set((state) => ({
          isChatInputVisible: resolveSetStateAction(value, state.isChatInputVisible),
        }));
      },
      ...
      setPendingDropData: (value) => {
        set((state) => ({
          pendingDropData: resolveSetStateAction(value, state.pendingDropData),
        }));
      },
      ...
      setIsHistoryPanelOpen: (value) => {
        set((state) => ({
          isHistoryPanelOpen: resolveSetStateAction(value, state.isHistoryPanelOpen),
        }));
      },
      setPlayingMessageIndex: (value) => {
        set((state) => ({
          playingMessageIndex: resolveSetStateAction(value, state.playingMessageIndex),
        }));
      },
      setLoadingMessageIndex: (value) => {
        set((state) => ({
          loadingMessageIndex: resolveSetStateAction(value, state.loadingMessageIndex),
        }));
      },
      setIsDraggingButton: (value) => {
        set((state) => ({
          isDraggingButton: resolveSetStateAction(value, state.isDraggingButton),
        }));
      },
      setIsDraggingModel: (value) => {
        set((state) => ({
          isDraggingModel: resolveSetStateAction(value, state.isDraggingModel),
        }));
      },
      setIsDragOverChat: (value) => {
        set((state) => ({
          isDragOverChat: resolveSetStateAction(value, state.isDragOverChat),
        }));
      },
      setButtonPosition: (value) => {
        set((state) => ({
          buttonPosition: resolveSetStateAction(value, state.buttonPosition),
        }));
      },
      setModelOverlayPos: (value) => {
        set((state) => ({
          modelOverlayPos: resolveSetStateAction(value, state.modelOverlayPos),
        }));
      },
      ...
      setSavedModelPosition: (value) => {
        set((state) => ({
          savedModelPosition: resolveSetStateAction(value, state.savedModelPosition),
        }));
      },
      toggleChat: () => {
      ...
      clearChat: async () => {
        ...
        if (state.isTempChat && state.currentChatId) {
          try {
            await chatHistoryService.deleteChat(state.currentChatId);
            Logger.log("AppStore", "Temp chat deleted:", state.currentChatId);
          } catch (error) {
      ...
      loadChatFromHistory: async ({ chatId }) => {
        ...
        try {
          ...
          if (fullChat.chatServiceData) {
            ...
            set({ chatMessages: toChatMessageItems(ChatService.getMessages()) });
          } else if (fullChat.messages) {
            ChatService.setMessages(
              toChatMessageInputsFromHistory(fullChat.messages as HistoryMessage[]),
            );
            set({ chatMessages: toChatMessageItems(ChatService.getMessages()) });
          }
      ...
      editUserMessage: async (
        ...
        try {
          ...
          const conversationContext = updatedMessages.slice(
            0,
            updatedMessages.findIndex((message) => message.id === newMessageId) + 1,
          );
      ...
      previousBranch: (messageId) => {
        try {
          ...
          Logger.error("AppStore", "Failed to navigate to previous branch:", error);
        }
      },
      nextBranch: (messageId) => {
        try {
          ...
          Logger.error("AppStore", "Failed to navigate to next branch:", error);
        }
      },
      startButtonDrag: () => {
      ...
      handleTranslate: async (text, sourceLanguage, targetLanguageOverride) => {
        const aiConfig = useConfigStore.getState().aiConfig;
        ...
        return TranslatorServiceProxy.translate(text, sourceLang, targetLanguage);
      },
      ...
      reloadScene: () => {
        Logger.log("AppStore", "Reloading 3D scene - clearing saved position");
        set((state) => ({
          savedModelPosition: null,
      ...
      forceChatOnlyMode: (reason = "3d-scene-error") => {
        ...
        configStore.updateUIConfig("enableModelLoading", false, { debounceMs: 0 });
...
export const syncAppSpeakingFromConversationState = (state: string) => {
  useAppStore.setState({ isSpeaking: state === ConversationStates.SPEAKING });
};
