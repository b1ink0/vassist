...
contextBridge.exposeInMainWorld("electron", {
  ...
  window: {
    ...
    setScaleFactor: (scaleFactor: number, scaleFactorHeight?: number) =>
      ipcRenderer.invoke(
  ...
  // Shortcuts
  shortcuts: {
    register: (shortcuts: Record<string, unknown>) =>
    ...
    onToggleModel: (callback: GenericCallback) => {
      ...
      return () =>
        ipcRenderer.removeListener("shortcut:toggle-model", callback);
    },
  },
  ...
  server: {
    start: (options: Record<string, unknown>) =>
      ipcRenderer.invoke("server:start", options),
  ...
  llm: {
    listModels: (customPath?: string | null) =>
      ipcRenderer.invoke("llm:list-models", customPath),
    downloadModel: (url: string, customPath?: string | null) =>
      ipcRenderer.invoke("llm:download-model", url, customPath),
    pullModel: (modelName: string, customPath?: string | null) =>
      ipcRenderer.invoke("llm:pull-model", modelName, customPath),
    searchOllamaModels: (query: string, page = 1, pageSize = 20) =>
      ipcRenderer.invoke("llm:search-ollama-models", query, page, pageSize),
    listOllamaModelTags: (
      modelId: string,
      query = "",
      page = 1,
    ...
    deleteModel: (filename: string, customPath?: string | null) =>
      ipcRenderer.invoke("llm:delete-model", filename, customPath),
    importModel: (sourcePath: string, customPath?: string | null) =>
      ipcRenderer.invoke("llm:import-model", sourcePath, customPath),
    ...
    onDownloadProgress: (callback: GenericCallback) => {
      ...
      return () =>
        ipcRenderer.removeListener("llm:download-progress", subscription);
    },
    getBackendStatus: (backend: string) =>
      ipcRenderer.invoke("llm:backend-status", backend),
    installBackend: (backend: string) =>
      ipcRenderer.invoke("llm:backend-install", backend),
    cancelBackendInstall: () =>
      ipcRenderer.invoke("llm:backend-cancel-install"),
  ...
  gptSovitsSetup: {
    start: (options: Record<string, unknown> = {}) =>
      ipcRenderer.invoke("gptsovits:setup:start", options),
    ...
    onLog: (callback: GenericCallback) => {
      ...
      return () =>
        ipcRenderer.removeListener("gptsovits:setup:log", subscription);
    },
    onComplete: (callback: GenericCallback) => {
      ...
      return () =>
        ipcRenderer.removeListener("gptsovits:setup:complete", subscription);
    },
  },
  ...
  whisperSetup: {
    ...
    onLog: (callback: GenericCallback) => {
      ...
      return () =>
        ipcRenderer.removeListener("whisper:setup:log", subscription);
    },
    onComplete: (callback: GenericCallback) => {
      const subscription = (_event: IpcRendererEvent, result: unknown) =>
        callback(result);
      ...
      return () =>
        ipcRenderer.removeListener("whisper:setup:complete", subscription);
    },
  },
});
