...
class SetupRunner {
  ...
  async bootstrap() {
    ...
    const bootstrap = new PythonBootstrap(
      (message: string) => {
  ...
  async runSetup(options: SetupOptions = {}) {
    ...
    const selectedBackend = (options?.torchBackend || "auto")
      .toString()
      .trim()
      .toLowerCase();
   ...
   */
  async run(
    logCallback: (logData: SetupLog) => void,
    options: SetupOptions = {},
  ) {
    this.logCallback = logCallback;
    this.cancelled = false;
    this.torchBackend = (options?.torchBackend || "auto")
      .toString()
      .trim()
      .toLowerCase();
    ...
    try {
      ...
      this.log({
        type: "info",
        message: "Estimated time: 10-30 minutes depending on internet speed\n",
      });
      this.log({
        type: "info",
        message: `Selected PyTorch backend: ${this.torchBackend}\n`,
      });
      this.log({ type: "info", message: "=".repeat(60) + "\n\n" });
