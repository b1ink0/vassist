...
export function createGetModelsDir({ app, fs, path, baseDir }: ModelDirDeps) {
  const ensureDirectory = (dirPath: string): string => {
    if (fs.existsSync(dirPath)) {
      ...
      if (!stat.isDirectory()) {
        throw new Error(
          `Models path exists but is not a directory: ${dirPath}`,
        );
      }
      return dirPath;
    }
    ...
    if (customPath && typeof customPath === "string") {
      ...
      if (normalizedCustomPath) {
        if (fs.existsSync(normalizedCustomPath)) {
          ...
          console.warn(
            "[LLM] customModelsPath points to a file, using parent directory:",
            parentDir,
          );
...
export function registerLLMHandlers({
      ...
      try {
        ...
        const items = tagMatches
          ...
          .filter((tag) => {
            ...
            return (
              !normalizedQuery ||
              fullValue.includes(normalizedQuery) ||
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        params.set(
          "limit",
          String(Math.max(1, Math.min(50, Number(pageSize) || 20))),
        );
          ...
          .map((entry) => {
            const description = [
              ...
              typeof entry.likes === "number"
                ? `${entry.likes.toLocaleString()} likes`
                : null,
            ...
            return {
              ...
              secondaryLabel:
                Array.isArray(entry.tags) && entry.tags.includes("gguf")
                  ? "GGUF"
                  : "Model repo",
              ...(typeof entry.downloads === "number"
        ...
        const total =
          typeof totalHeader === "string" ? Number(totalHeader) || null : null;
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Hugging Face file listing failed: ${response.status}`,
          );
        }
  ...
  ipcMain.handle(
    "llm:list-models",
    async (_event: IpcMainInvokeEvent, customPath: string | null = null) => {
      try {
        ...
        const models = files
          .filter(
            (file: string) =>
              file.endsWith(".gguf") && !file.startsWith("mmproj-"),
    ...
    async (
      ...
      try {
        ...
        for (const layer of layers) {
          ...
          if (
            mediaType.includes("projector") ||
            mediaType === "application/vnd.ollama.image.projector"
          ) {
            mmprojLayers.push(layer);
            console.log(
              "[LLM] Detected mmproj layer:",
        ...
        if (downloadedMmprojFiles.length > 0) {
          ...
          console.log(
            "[LLM] Vision encoder saved:",
            mmprojFileName,
            `(${(largestMmprojFile.size / 1024 / 1024).toFixed(1)}MB)`,
          );
        }
        ...
        for (const file of [
          ...
          ...downloadedMmprojFiles,
        ]) {
          if (
            file.path !== largestModelFile.path &&
            (!downloadedMmprojFiles.length ||
              file.path !==
                downloadedMmprojFiles.find(
                  (f) =>
                    f.size ===
                    Math.max(...downloadedMmprojFiles.map((f) => f.size)),
                )?.path) &&
        ...
        console.log(
          ...
          finalModelName,
          `(${(largestModelFile.size / 1024 / 1024).toFixed(1)}MB)`,
        );
    ...
    async (
      ...
      try {
        ...
        if (!filename.endsWith(".gguf")) {
          return {
            success: false,
            error: "Invalid file: must be a .gguf model file",
          };
        }
          ...
          try {
            ...
            if (match) {
              ...
              if (!org || !repo || !branch) {
                return {
                  success: false,
                  error: "Unable to parse Hugging Face URL.",
                };
              }
              ...
              if (apiRes.ok) {
                ...
                if (mmprojFile) {
                  ...
                  try {
                    ...
                    if (fallbackRes.ok) {
                      const fallbackFiles =
                        (await fallbackRes.json()) as HuggingFaceTreeEntry[];
                      const fallbackMmproj = fallbackFiles.find(
                        (f) =>
                          typeof f.path === "string" &&
                          /mmproj.*\.gguf$/i.test(f.path),
                      );
                    ...
                    console.log(
                      "[LLM] No mmproj in fallback repo:",
            ...
            console.log(
              "[LLM] Could not check for mmproj:",
        ...
        if (mmprojUrl && mmprojFilename) {
          ...
          await new Promise((resolve, reject) => {
            https
              .get(mmprojUrl, (response: IncomingMessage) => {
                if (
                  response.statusCode === 302 ||
                  response.statusCode === 301
                ) {
              ...
              .on("error", (err: Error) => {
                console.warn(
                  "[LLM] Failed to download mmproj (non-critical):",
                  err.message,
                );
          ...
          console.log(
            "[LLM] Downloaded with vision support:",
            filename,
            "+",
            mmprojFilename,
          );
  ...
  ipcMain.handle(
    "llm:backend-status",
    async (_event: IpcMainInvokeEvent, backend = "auto") => {
      try {
  ...
  ipcMain.handle(
    "llm:backend-install",
    async (event: IpcMainInvokeEvent, backend = "auto") => {
      try {
        if (!llmBackendManager) {
          return { success: false, error: "LLM backend manager unavailable" };
        }
  ...
  ipcMain.handle("llm:backend-cancel-install", async () => {
    ...
    return {
      success: false,
      error: "Cancel is not supported for backend installation yet",
    };
  });
}
...
function downloadFile(
  ...
  response.on("data", (chunk: Buffer) => {
    ...
    const downloadPercent =
      Math.round((downloadedSize / totalSize) * 1000) / 10;
    const actualPercent =
      startPercent + (downloadPercent / 100) * (maxPercent - startPercent);
    ...
    event.sender.send("llm:download-progress", {
      ...
      status:
        fileType === "mmproj"
          ...
          : `Download complete: ${path.basename(filePath)}`,
    });
