...
const toTranslatorConfigureInput = (
  config: MessageData,
): TranslatorConfigureInput => {
...
async function registerHandlers() {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CONFIG_SAVE,
    async (message) => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CONFIG_EXISTS,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CONFIG_REMOVE,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CONFIG_GET_ALL,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CONFIG_CLEAR,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_SETTINGS_SAVE,
    async (message) => {
      const key = asString(message.data.key);
      const value = message.data.value;
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_SETTINGS_LOAD,
    async (message) => {
      const key = asString(message.data.key);
      const defaultValue = message.data.defaultValue;
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_SETTINGS_EXISTS,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_SETTINGS_REMOVE,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_SETTINGS_CLEAR,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_SAVE,
    async (message) => {
      const key = asString(message.data.key);
      const value = message.data.value;
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_LOAD,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_EXISTS,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_REMOVE,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_GET_ALL,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_CLEANUP,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CACHE_CLEAR,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CHAT_SAVE,
    async (message) => {
      ...
      try {
        ...
        Logger.log(
          "Background",
          "STORAGE_CHAT_SAVE success, saved to IndexedDB:",
          chatId,
        );
        return result;
      } catch (error) {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CHAT_LOAD,
    async (message) => {
      const chatId = asString(message.data.chatId);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CHAT_EXISTS,
    async (message) => {
      const chatId = asString(message.data.chatId);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CHAT_REMOVE,
    async (message) => {
      const chatId = asString(message.data.chatId);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CHAT_GET_ALL,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_CHAT_CLEAR,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_FILE_SAVE,
    async (message) => {
      ...
      if (data && typeof data === "object") {
        ...
        if (Array.isArray(rawData.data)) {
          Logger.log(
            "Background",
            "Converting data.data Uint8Array back to Blob",
          );
          const blobType = asString(
            rawData._blobType,
        ...
        if (Array.isArray(rawData.motionData)) {
          Logger.log(
            "Background",
            "Converting motionData Uint8Array back to Blob",
          );
          const blobType = asString(
            rawData._motionBlobType,
            "application/octet-stream",
          );
        ...
        if (Array.isArray(rawData.modelData)) {
          Logger.log(
            "Background",
            "Converting modelData Uint8Array back to Blob",
          );
          const blobType = asString(
            rawData._modelBlobType,
            "application/octet-stream",
          );
      ...
      try {
        const result = await storageManager.files.save(
          fileId,
          storageData,
          category,
        );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_FILE_LOAD,
    async (message) => {
      const fileId = asString(message.data.fileId);
      const result = (await storageManager.files.load(
        fileId,
      )) as FileRecord | null;
      ...
      if (result && typeof result === "object") {
        ...
        if (result.data instanceof Blob) {
          Logger.log(
            "Background",
            "Converting data.data Blob to Uint8Array for transfer",
          );
        ...
        if (result.motionData instanceof Blob) {
          Logger.log(
            "Background",
            "Converting motionData Blob to Uint8Array for transfer",
          );
        ...
        if (result.modelData instanceof Blob) {
          Logger.log(
            "Background",
            "Converting modelData Blob to Uint8Array for transfer",
          );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_FILE_EXISTS,
    async (message) => {
      const fileId = asString(message.data.fileId);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_FILE_REMOVE,
    async (message) => {
      const fileId = asString(message.data.fileId);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_FILES_GET_ALL,
    async () => {
        ...
        for (const [_fileId, entry] of Object.entries(files)) {
          ...
          if (fileData && typeof fileData === "object") {
            ...
            if (fileData.motionData instanceof Blob) {
              const blobType =
                fileData.motionData.type || "application/octet-stream";
            ...
            if (fileData.modelData instanceof Blob) {
              const blobType =
                fileData.modelData.type || "application/octet-stream";
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_FILES_CLEAR,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_DATA_SAVE,
    async (message) => {
      const key = asString(message.data.key);
      const value = message.data.value;
      const category = asString(message.data.category);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_DATA_LOAD,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_DATA_EXISTS,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_DATA_REMOVE,
    async (message) => {
      const key = asString(message.data.key);
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_DATA_GET_ALL,
    async () => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.STORAGE_DATA_CLEAR,
    async () => {
  ...
  backgroundBridge.registerHandler(
    ...
    async (_message, _sender, tabId) => {
      if (tabId) {
        ...
        Logger.log(
          "Background",
          `TAB_INIT: Auto-configuring services for tab ${tabId}...`,
        );
        try {
          const aiConfig = (await storageManager.config.load(
            "aiConfig",
            DefaultAIConfig,
          )) as typeof DefaultAIConfig;
          const ttsConfig = (await storageManager.config.load(
            "ttsConfig",
            DefaultTTSConfig,
          )) as typeof DefaultTTSConfig;
          const sttConfig = (await storageManager.config.load(
            "sttConfig",
            DefaultSTTConfig,
          )) as typeof DefaultSTTConfig;

          if (aiConfig.provider) {
            await aiService.configure(aiConfig, tabId);
            Logger.log(
              "Background",
              `TAB_INIT: AI service configured for tab ${tabId}`,
            );
          }
          ...
          if (
            ...
            await translatorService.configure(
              toTranslatorConfigureInput(aiConfig as MessageData),
              tabId,
            );
            Logger.log(
              "Background",
              `TAB_INIT: Translator configured for tab ${tabId}`,
            );
          }
          ...
          if (
            ...
            Logger.log(
              "Background",
              `TAB_INIT: LanguageDetector configured for tab ${tabId}`,
            );
          }
          ...
          if (
            ...
            Logger.log(
              "Background",
              `TAB_INIT: Summarizer configured for tab ${tabId}`,
            );
          }
          ...
          if (aiConfig.aiFeatures?.rewriter?.enabled !== false) {
            ...
            Logger.log(
              "Background",
              `TAB_INIT: Rewriter configured for tab ${tabId}`,
            );
          }
          ...
          if (aiConfig.aiFeatures?.writer?.enabled !== false) {
            ...
            Logger.log(
              "Background",
              `TAB_INIT: Writer configured for tab ${tabId}`,
            );
          }
          ...
          if (ttsConfig.enabled) {
            ...
            Logger.log(
              "Background",
              `TAB_INIT: TTS configured for tab ${tabId}`,
            );
          }
          ...
          if (sttConfig.enabled) {
            ...
            Logger.log(
              "Background",
              `TAB_INIT: STT configured for tab ${tabId}`,
            );
          }
        } catch (error) {
          Logger.warn(
            "Background",
            `TAB_INIT: Auto-configure failed for tab ${tabId}:`,
            error,
          );
        }
  ...
  backgroundBridge.registerHandler(
    ...
    async (message) => {
      const config = asMessageData(message.data.config) as {
      ...
      try {
        ...
        if (!ChromeAIValidator.isSupported()) {
          ...
          return {
            success: false,
            message:
              "Chrome AI is not supported. Please check Chrome version and flags.",
          };
        }
        ...
        const progressCallback = (progress: object) => {
          ...
          chrome.tabs
            ...
            .catch((err) =>
              Logger.error(
                "Background",
                "Failed to send Chrome AI progress:",
                err,
              ),
            );
        };
        ...
        Logger.log(
          "Background",
          "Download completed or session created successfully",
        );
        return {
          success: true,
          message:
        ...
        return {
          success: false,
          message:
            messageFromError(error instanceof Error ? error : String(error)) ||
            'Failed to start download. Please manually check chrome://components and click "Check for update" on "Optimization Guide On Device Model".',
        };
      }
    },
  );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      Logger.log(
        "Background",
        "TTS_GENERATE_SPEECH called for tab:",
        tabId,
        "text:",
        text.substring(0, 50),
      );
      ...
      if (provider === "kokoro") {
        Logger.log(
          "Background",
          "Kokoro provider detected, using offscreen directly",
        );
        ...
        try {
          ...
          if (statusData.initialized !== true) {
            Logger.log(
              "Background",
              "Kokoro not initialized, auto-initializing...",
            );
            ...
            try {
              ...
              const initResult =
                await offscreenManager.sendToOffscreen(initMessage);
          ...
          Logger.error(
            "Background",
            "Kokoro auto-initialization failed:",
            initError,
          );
          throw new Error(
            `Kokoro initialization failed: ${messageFromError(initError instanceof Error ? initError : String(initError))}`,
          );
        }
        ...
        try {
          const kokoroMessage = {
            ...
            data: {
              ...
              speed:
                tabState.config?.speed !== undefined
                  ? tabState.config.speed
                  : 1.0,
            },
          };
          ...
          if (resultData.audioBuffer instanceof ArrayBuffer) {
            resultData.audioBuffer = Array.from(
              new Uint8Array(resultData.audioBuffer),
            );
            Logger.log(
              "Background",
              "Converted Kokoro audioBuffer to Array:",
              asNumber((resultData.audioBuffer as number[]).length),
            );
          }

          return {
            audioBuffer: resultData.audioBuffer ?? null,
            mimeType: "audio/wav",
          };
        } finally {
      ...
      if (!result) {
        Logger.warn(
          "Background",
          "TTS_GENERATE_SPEECH returned null/undefined - generation failed or stopped",
        );
      ...
      Logger.log(
        "Background",
        "Converted ArrayBuffer to Array:",
        audioData.length,
        "bytes",
      );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.TTS_STOP_GENERATION,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    MessageTypes.TTS_RESUME_GENERATION,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    MessageTypes.TTS_STOP_PLAYBACK,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    MessageTypes.TTS_RESUME_PLAYBACK,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, sender, tabId) => {
        ...
        if (offscreenResponse.type === MessageTypes.KOKORO_DOWNLOAD_PROGRESS) {
          ...
          chrome.tabs
            ...
            .catch((err) =>
              Logger.error(
                "Background",
                "Failed to send Kokoro progress:",
                err,
              ),
            );
      ...
      try {
        ...
        Logger.log(
          "Background",
          "Warming up Kokoro model with test generation...",
        );
        try {
          ...
          Logger.warn(
            "Background",
            "Kokoro model warmup failed (continuing anyway):",
            warmupError,
          );
        }
      ...
      try {
        ...
        if (resultData.audioBuffer instanceof ArrayBuffer) {
          ...
          Logger.log(
            "Background",
            "Converted Kokoro audioBuffer to Array:",
            (resultData.audioBuffer as number[]).length,
          );
        }
  ...
  backgroundBridge.registerHandler(
    ...
    async (message) => {
      ...
      Logger.log(
        "Background",
        "KOKORO_CHECK_STATUS - Received from offscreen:",
    ...
    async (message) => {
      ...
      try {
        ...
        if (!Array.isArray(audioBuffer)) {
          throw new Error(
            "audioBuffer must be an Array, got: " + typeof audioBuffer,
          );
        }

        Logger.log(
          "Background",
          "Forwarding Array to offscreen:",
          audioBuffer.length,
          "bytes",
        );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `STT_TRANSCRIBE_AUDIO: converted Array (${audioBuffer.length} bytes) to ArrayBuffer`,
      );

      const text = await sttService.transcribeAudio(
        arrayBuffer,
        mimeType,
        tabId,
      );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      const config = asMessageData(message.data.config);
      await translatorService.configure(
        toTranslatorConfigureInput(config),
        tabId,
      );
      return { configured: true };
    },
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `TRANSLATOR_TRANSLATE (${sourceLanguage}->${targetLanguage}):`,
        text.substring(0, 50),
      );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `TRANSLATOR_TRANSLATE_STREAMING (${sourceLanguage}->${targetLanguage}):`,
        text.substring(0, 50),
      );
      ...
      for await (const chunk of translatorService.translateStreaming(
        text,
        sourceLanguage,
        targetLanguage,
        tabId,
        ...
        chrome.tabs
          ...
          .catch((err) =>
            Logger.error(
              "Background",
              "Failed to send translation stream token:",
              err,
            ),
          );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.TRANSLATOR_DESTROY,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    MessageTypes.LANGUAGE_DETECTOR_IS_CONFIGURED,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    ...
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
      const availability =
        await languageDetectorService.checkAvailability(tabId);
      return { availability };
    },
  );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.LANGUAGE_DETECTOR_DESTROY,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    MessageTypes.SUMMARIZER_IS_CONFIGURED,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `SUMMARIZER_SUMMARIZE (${options?.type || "tldr"}):`,
        text.substring(0, 50),
      );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `SUMMARIZER_SUMMARIZE_STREAMING (${options?.type || "tldr"}):`,
        text.substring(0, 50),
      );
      ...
      for await (const chunk of summarizerService.summarizeStreaming(
        text,
        options,
        tabId,
      )) {
        ...
        chrome.tabs
          ...
          .catch((err) =>
            Logger.error(
              "Background",
              "Failed to send summary stream token:",
              err,
            ),
          );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.SUMMARIZER_DESTROY,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    MessageTypes.REWRITER_IS_CONFIGURED,
    async (_message, _sender, tabId) => {
      if (!tabId) throw new Error("Tab ID required");
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `REWRITER_REWRITE (${options?.tone || "as-is"}):`,
        text.substring(0, 50),
      );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `REWRITER_REWRITE_STREAMING (${options?.tone || "as-is"}):`,
        text.substring(0, 50),
      );
      ...
      for await (const chunk of rewriterService.rewriteStreaming(
        text,
        options,
        tabId,
      )) {
        ...
        chrome.tabs
          ...
          .catch((err) =>
            Logger.error(
              "Background",
              "Failed to send rewrite stream token:",
              err,
            ),
          );
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, _sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `WRITER_WRITE (${options?.tone || "neutral"}):`,
  ...
  backgroundBridge.registerHandler(
    ...
    async (message, sender, tabId) => {
      ...
      Logger.log(
        "Background",
        `WRITER_WRITE_STREAMING (${options?.tone || "neutral"}):`,
        prompt.substring(0, 50) || "no prompt",
      );
      ...
      for await (const chunk of writerService.writeStreaming(
        prompt,
        options,
        tabId,
      )) {
        ...
        chrome.tabs
          ...
          .catch((err) =>
            Logger.error(
              "Background",
              "Failed to send write stream token:",
              err,
            ),
          );
  ...
  backgroundBridge.registerHandler(
    MessageTypes.OFFSCREEN_AUDIO_PLAY,
    async (message) => {
  ...
  backgroundBridge.registerHandler(
    MessageTypes.OFFSCREEN_AUDIO_STOP,
    async (message) => {
...
Logger.init(storageManager)
  .then(() => {
    console.log("Background: Logger initialized");
  })
  .catch((error) => {
    console.warn(
      "Background: Logger failed to initialize, using defaults:",
      error,
    );
  });
