...
function downloadFile(url: string, destPath: string): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    https
      .get(url, { headers: { "User-Agent": "vassist-build" } }, (response) => {
...
async function downloadModels(): Promise<void> {
  ...
  for (const [name, config] of Object.entries(
    ...
    console.log(
      `[android-models] Processing ${name.toUpperCase()} (${config.size})`,
    );
    ...
    const files = config.files.map((f) =>
      typeof f === "string" ? { src: f, dest: f } : f,
    );
    const allExtracted = files.every((f) =>
      fs.existsSync(path.join(cacheModelDir, f.dest)),
    );
    ...
    if (!allExtracted) {
      try {
        ...
        console.error(
          `[android-models] ❌ Extraction failed: ${getErrorMessage(error instanceof Error ? error : String(error))}`,
        );
  ...
  return {
    ...
    async buildStart() {
      ...
      try {
        if (areModelsCached()) {
          console.log(
            "[android-models] ✓ Models already cached, skipping download",
          );
        } else {
